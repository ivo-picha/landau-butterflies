...
@showprogress for Y in Y_list
    for ky0 in ky_list
        ...
        for j in eachindex(evalsH)
            global states_vec
            push!(states_vec, (evalsH[j], Y, ky0, evecsH[:,j]))
        end
...
N_uc_x = 2                 # number of unit lengths to be plotted in x
N_uc_y = 2
Nppuc = 15                  # number of points per unit length
